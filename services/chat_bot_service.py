...
class ChatBot:
    ...
    # ---------------- STEP 1: AUTO-DETECT RESUME UPLOAD ----------------
        # Check if user just uploaded a resume or wants automatic analysis
        auto_analysis_triggers = [
            "analyze", "upload", "resume", "check", "evaluate", "review", 
            "score", "feedback", "suggestions", "improve", "skills"
        ]
        
        # Auto-trigger analysis for common requests
        if any(trigger in question.lower() for trigger in auto_analysis_triggers):
            question = "Please analyze my resume and provide detailed feedback"

    # ---------------- STEP 2: Greeting HARD STOP ----------------
        greetings = ["hi", "hii", "hello", "hey", "thanks", "thank you"]

        if question.lower() in greetings:
            return success("👋 Hello! I'm your smart resume analyzer. Upload your resume and I'll automatically analyze it with personalized feedback!")

    # ---------------- STEP 3: Empty question guard ----------------
        if not question or len(question) < 2:
            return success("📄 Upload your resume and I'll provide instant analysis with score, skills assessment, and personalized suggestions!")
