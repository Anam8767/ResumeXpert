...
@router.get("/{id}/analysis", dependencies=[Depends(get_current_token)])
async def get_resume_analysis(id: str):
    # Get namespace_id from bot id
    bot = KnowledgeBot.objects(id=ObjectId(id)).first()
    if not bot:
        return error("Bot not found")
    
    return await chatBotService.get_resume_analysis(bot.namespace_id)