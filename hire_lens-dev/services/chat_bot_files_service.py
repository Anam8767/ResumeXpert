...
class ChatbotFilesService:
    ...
    async def delete_file(
        ...
        print("DELETE HIT:", file_id, namespace_id, chatbot_id)

    # 1️⃣ DB se file nikalo (name chahiye pinecone ke liye)
        file_obj = KnowledgeBotFiles.objects(
            id=ObjectId(file_id),
            chatbot_id=chatbot_id
    ).first()
    ...
    # 2️⃣ DB record delete
        KnowledgeBotFiles.objects(id=ObjectId(file_id)).delete()

    # 3️⃣ Pinecone delete (BACKGROUND)
        backgroundTasks.add_task(
            self.pineconeService.delete_vectorized_docs,
            namespace_id,
            "name",
            [file_name]
    )
