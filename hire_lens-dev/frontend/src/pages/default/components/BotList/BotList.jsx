...
const BotList = () => {
  ...
  const handleCreate = async (e) => {
    ...
    if (!formData.bot_name || !formData.description) {
  ...
  return (
    <div className="hl-dashboard">
      {/* CREATE BOT */}
      <div className="hl-card create-bot">
      ...
      {/* BOT LIST */}
      <div className="hl-bots">
        {bots.length === 0 ? (
          <p className="empty-text">No chatbots created yet.</p>
        ) : (
          bots.map((bot) => (
            <div key={bot._id["$oid"]} className="hl-bot-card">
              <h4>{bot.bot_name}</h4>
              <p>{bot.description}</p>
